...
def gpsIsValid(lat, lon):
      invalid = [0,-1]
      return (lat not in invalid and lon not in invalid) and (lat >= -90 and lat <= 90) and (lon >= -180 and lon <= 180) and (lat >=-1 and lat <= 1) and (lon >=-1 and lon <= 1)
...
def getSduInfo(sdu_list, index):
      ...
      payload = sdu[1]
      print('#########', payload, '#########')
      print('#########', payload[7:14], int(payload[7:14],16), payload[14:22], int(payload[14:22],16),'#########')
      node_id_hex = sdu[2] # note intellect reports hex node id not dec as of 3/16/2016
      rx_timestamp = sdu[3]
      #gaowei
      gmtdate=convertToDateTime(rx_timestamp)
      loctime=utc2local(gmtdate)
      hh=int(payload[0:2],16)
      mm=int(payload[2:4],16)
      ss=int(payload[4:6],16)
      lat=int(payload[7:14],16)
      lng=int(payload[14:22],16)
      rlat=round(float(lat/1000000.000000),6)
      rlng=round(float(lng/1000000.000000),6)
      print '------------------------------------------------------------------------------'
      print 'Begin parsing ULSDU at %s\nsdu_id: %s\nnodeId: %s\nrx_timestamp: %s\nRaw payload: %s'\
      % (datetime.datetime.now(), sdu_id, hex(int(node_id_hex, 16)), rx_timestamp, payload)
      print "------------------------------------"
      print "Parse Data:", payload.decode('hex')
      print "Datetime:",loctime
      print "Lat:",rlat
      print "Lng:",rlng
      print "------------------------------------"
      print "ULSDU end\n"
      ...
      #Two URLs
      CONN_ADDR1 = 'dds-6nndd4f70e627b441340-pub.mongodb.rds.aliyuncs.com:3717'
      CONN_ADDR2 = 'dds-6nndd4f70e627b442118-pub.mongodb.rds.aliyuncs.com:3717'
      REPLICAT_SET = 'mgset-6170057'
      username = 'root'
      password = 'Linde123'
      #Retrieve mongoclient
      client = MongoClient([CONN_ADDR1, CONN_ADDR2], username='user',password='password',replicaSet=REPLICAT_SET)
  
      #Authorization. Here, the user is based on the admin database authorization.
      #client.admin.authenticate(username, password)

      db = client['snapmeter_dev']
      collection = db['logs']
      collection2 = db['locations']


      # todo: average the readings in a group or something
      # todo: regex the payload for integrity
      # if gps not valid in reading, check if exists in locations already
      try:
            GPS_VALID = gpsIsValid(rlat, rlng)
            # update location db with logic:
            print('GPS CHECK: ', rlat, rlng, gpsIsValid(rlat,rlng))
            if GPS_VALID:
                  print('cool valid gps btw')
                  collection2.update(
                        {"name": sdu[2]},
            ...
            decoded = payload.decode('hex').strip()
            print ('DECODED:', decoded)
            m = re.search("\d{1,}\.\d{1,}", decoded)
            value = np.median([int(x) for x in decoded[m.start():].split()[1::2]])
            ...
            if GPS_VALID:
                  ...
                  # try find last known value in location storage
                  cur = collection2.find({'name': str(sdu[2])})
                  print ('CURSOR', cur, str(sdu[2]))
...
while cfg['useEndDatetime'] != '1' or not endTimeReached:
   try:
      ...
      if status != 'OK':
         ...
         #print 'done sleep 10sec on error'
      elif size == 0 and last_sdu_id == '':
      	 #print 'no new sdu,wait 3sec and then go on'
         time.sleep(3) # queue is empty, attempt to get latest SDUs every 1 second
      elif size == 0:
